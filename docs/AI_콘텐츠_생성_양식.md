...
1. **양식 복사** — 아래에서 만들고 싶은 도메인(예: 주요 질환)의 "AI에게 주는 글" 블록을 복사한다.
2. **AI에게 요청** — AI 채팅창에 붙여넣고, 맨 끝에 무엇을 만들지 적는다. 예: `→ "심근경색, 폐렴, 당뇨병 3개 만들어줘"`. AI가 JSON을 출력한다.
3. **앱에 가져오기** — JSON을 `.json` 파일로 저장 → 앱 **데이터 관리 → JSON 백업 복원**에서 그 파일 선택 → **가져오기 방식 = `신규만 추가`** 선택 → 적용.
   - **용어(terms)** 는 더 쉬운 방법도 있음: JSON 대신 **표(TSV) 붙여넣기** (아래 A-2). 파일 저장 없이 바로 붙여넣기 가능.
...
> 이 양식은 앱의 실제 데이터 구조(필드명·선택 목록)에서 추출해 만들었습니다. 앱이 업데이트되어 선택 목록이 바뀌면 이 문서도 함께 갱신하세요. (앱 안 **"AI 콘텐츠 양식 복사"** 버튼은 항상 최신 목록으로 자동 생성됩니다.)
