...
- **AI 생성 콘텐츠 양식에는 "언어 규칙"과 "다중 출력 형식"을 명시한다(v1.03).** 설명류(usmle 포인트·약물 계열·질환 임상 포인트·1부 정리)는 **한국어 기본**, 단 언어 고정 칸(def_en=영어/def_uz=우즈벡)과 고유표기(용어명·약물명·학명·약어·발음기호)는 **원어 유지**로 못 박는다 — 안 그러면 AI가 영어로 새거나 섞는다. 또 가져오기 형식은 한 가지로 강제하지 말고 **TSV·JSON 둘 다 출력**하게 해 사용자가 편한 쪽을 붙여넣게 한다(둘 다 자동 인식). 단 "둘 중 하나만 붙여넣기"를 양식에 명시.
