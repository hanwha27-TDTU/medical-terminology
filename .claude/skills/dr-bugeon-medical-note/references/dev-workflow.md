...
- **외부 코드 리뷰(Codex 등)는 "설계 의도"로 한 번 거른 뒤 반영한다.** v1.06에서 외부 리뷰 6건을 받았는데, **진짜 수확은 사각지대(P1-B: 부분 도메인 로드 실패 시 `canonicalVersion` 전진 → 부활 위험)를 짚어준 것**이었다. 반면 일부 지적은 이 앱의 의도된 설계를 모른 채 일반 규칙을 적용해 **과대평가**였다: ① fence를 "수정 이후(OR)"로 넓히라 → 부활 방지 위해 `createdAt` 기반 AND를 의도적으로 유지(storage-sync §5.3). ② "기준본 변경 시 노트도 업로드 금지" → 노트는 4도메인 canonical 모델이 아니라 **독립 newest-wins + 충돌사본 + tombstone 모델**이라 'drop' 개념이 없어 손실 없음. → **결론: 외부 리뷰는 신선한 눈으로 받되, 각 지적을 설계 의도(부활 방지 우선 · 노트 독립 모델 등)로 검증해 "진짜 결함 vs 의도된 트레이드오프"를 가른다. 의도된 결정은 그 근거를 스킬/주석에 남겨 다음 리뷰 때 재논쟁을 줄인다.** 또한 문서와 코드가 어긋나면(예: §5.3 fence가 문서엔 OR, 코드엔 AND였음) 리뷰가 그걸 버그로 오인하니, 의도가 맞는 쪽으로 문서를 정렬한다.
- **거대 템플릿 리터럴 UI를 재배치할 땐 인라인 컨트롤을 손으로 다시 쓰지 말고 "잘라 재배치"한다.** 설정 모달(v1.05) 접기/펼치기+재정렬 시, `input id`·`onclick`·`value=${}`를 한 글자라도 바꾸면 동작이 깨질 위험이 커서, **파이썬으로 각 구획을 줄 범위로 잘라 순서만 바꾸고 `<details>/<summary>`로 감쌌다.** 검증: ① 모든 `input id`가 정확히 1회씩 존재, ② 섹션 제목 중복 0, ③ `<details>/<summary>/<div>` 태그 짝 균형, ④ `node --check`. 토글되는 요소(예: `onAIRouteChange`가 `display`를 바꾸는 박스)는 **id가 붙은 원본 마크업을 그대로** 묶음 안에 두면 콜백이 그대로 동작한다. 접기 UI는 JS 없이 **네이티브 `<details>`**(요약 클릭=토글)로 충분하다.
- **임상(의학) 개념은 필드 key·기존 라벨로 짐작하지 말고 사용자(전문가)에게 확인한다.** 질환 `chief_complaints`를 "주증상(증상의 부분집합)"으로 라벨링했다가, 실제로는 **주호소(Chief Complaint = 내원 이유, 증상이 아닐 수도 있음 · 주호소 ⊃ 주증상)** 임이 드러나 정정했다(v1.18 오류 → v1.20). 증상(symptom·주관) vs 징후(sign·객관) vs 주호소(내원 이유) 같은 구분은 임의로 합치거나 "부분집합/같은 용어"로 강제하면 안 된다. → **개념이 모호하면 AskUserQuestion으로 정의를 확정**하고, 정정은 **표시 라벨·AI 양식 지침만 바꾸고 필드 key는 유지**해 기존 데이터를 보존한다(key 변경은 마이그레이션·데이터 위험).
- **"중복 검토/선택" 요구는 granularity(항목 vs 필드/섹션)를 먼저 확정한다.** "일부 유지·일부 변경"이 핵심이면 항목 단위로는 부족하다 — 처음부터 필드/섹션 단위로 잡는다(v1.15 항목단위→v1.17 필드단위 재작업). 상세는 import-backup-media §8.1.
- **새 필드 하나는 "한 PR에서 전 경로"로 끝내야 한다 — 안 그러면 며칠 뒤 신선한 눈 감사가 회귀를 줍는다(v1.36→v1.37).** `finding_type`(용어 선택 필드)를 추가하며 normalize·row·SQL·폼·표시까지는 했지만 **해시 payload·CSV 파일 import 경로·라이트 테마 색** 3곳을 빠뜨렸고, 직후 6차원 병렬 전수점검(보안 제외)에서 전부 잡혔다. 교훈: ① 도메인 필드 추가 후 `rg`로 **해시 payload·전 import 경로(파일 업로드 ALIAS 포함)·테마 색**까지 parity 체크리스트(import-backup §8.1.1)를 훑는다. ② 큰 변경 뒤에는 **dimension별 병렬 에이전트 감사**(parity/동기화/UI테마/가져오기/죽은코드)가 구현자 사각지대를 잘 잡는다 — 특히 *자기 직전 변경*의 회귀를. ③ 발견된 결함은 HIGH(내 회귀)부터 한 PR로 묶어 수정하고, 판단 필요건(미리보기 모드·죽은 기능·단방향 export)은 AskUserQuestion으로 방향을 받아 후속 처리한다.

### 2.6 기능 추가에서 얻은 좋은 패턴 (2026-06 — 권장)

- **파생물은 단일 진실원에서 동적 생성한다(하드코딩 금지).** AI 콘텐츠 양식을 `normalizeXForStorage({id:1})`의 키 + `DISEASE_SECTIONS` + 옵션 상수에서 생성 → 스키마(필드·섹션·드롭다운)가 바뀌면 양식이 **자동 추종**. 문서/버튼 같은 파생물이 stale 되지 않는다. **실증(v1.10~v1.11):** 질환 cause의 분류 옵션 정비·`origin`(상위분류) rowField 추가가 폼·드로어 표(`sec.rowFields` map)·통합검색 인덱스·CSV·AI양식까지 한 번에 자동 반영됐다. 새 분류/필드는 *상수/`rowFields`만* 고치고 렌더·내보내기 하드코딩이 없는지 `grep`로 확인한다.
- **분류 옵션 설계: "한 축으로, MECE, 다축은 필드 분리"(v1.10~v1.11).** 분류 드롭다운은 한 가지 축(기전이면 기전만)으로 유지하고 **다른 축을 평면에 섞지 않는다**(질환 원인에 장기계인 `신경학적`을 넣으면 `혈관성`↔`신경학적` 중복 → 제거). 옵션 명칭이 서로 포함관계면(`대사성` vs `내분비/대사성`) 갈라 중복을 없앤다(→`내분비성`). 또 다른 축이 정말 필요하면(예: 내인성/외인성) **평면 추가 대신 별도 필드**로 두고, 2분법이 대상을 완벽히 못 가르면(양다리·불명) **`혼합·불명` 버킷**으로 완전성(collectively exhaustive)을 확보한다. 행 단위 분류면 같은 항목도 행마다 다른 축값을 줄 수 있어 표현력이 optgroup보다 낫다. 라벨을 바꿀 땐 `*_MIGRATE` 맵으로 기존 데이터 값을 이관(orphan 방지).
- **Supabase 통합 SQL은 `SUPABASE_SCHEMA_SQL` 하나로 제공한다(한 번에 붙여넣기).** 개발자 화면에 복사 버튼이 있고, `create table if not exists` + `alter ... add column if not exists` + 최종본 메타 시드 + 끝에 진단 SELECT로 **멱등**이라 재실행 안전. 새 컬럼이 생기는 스키마 변경만 여기 반영하면 되고, **질환 상세 분류처럼 `body jsonb`에 들어가는 값은 SQL 변경이 불필요**하다(컬럼이 아니므로). RLS/policy는 두지 않는 개인용(anon + 기본 접근) 설계.
...
- **AI 생성 콘텐츠 양식에는 "언어 규칙"과 "다중 출력 형식"을 명시한다(v1.03·v1.08).** 설명류(usmle 포인트·MOA/기전·임상 사용·부작용·약물 계열·질환 임상 포인트·1부 정리)는 **한국어를 주 서술 언어로** 하고, 그 안의 **의학용어는 반드시 한국어(영어) 형식으로 병기**한다(예: 서맥(bradycardia), 방실차단(AV block)) — "한국어 기본"만 적으면 AI가 영어 용어를 그대로 나열하므로 병기 형식·예시를 구체적으로 박아야 한다(v1.08). 단 언어 고정 칸(def_en=영어/def_uz=우즈벡)은 병기 없이 해당 언어만, 고유표기(약물명·학명·발음기호·순수 약어 PO/IV)는 원어 유지. 또 가져오기 형식은 한 가지로 강제하지 말고 **TSV·JSON 둘 다 출력**하게 해 사용자가 편한 쪽을 붙여넣게 한다(둘 다 자동 인식, "둘 중 하나만 붙여넣기" 명시). **2턴 프로토콜(v1.62):** 양식만 붙여넣었을 때 AI가 바로 예시를 뱉지 않도록, `buildAIContentSpec`의 `header` 맨 앞에 "지금은 지시문만 받은 상태 → '준비됐습니다. 만들 항목과 개수를 알려주세요'라고만 답하고 대기 → 다음 메시지로 항목을 받으면 그때 [1부]+[2부] 출력" 프로토콜을 박는다(실사용은 양식 먼저·항목 나중). `header`는 모든 도메인 spec이 공유하므로 한 곳만 고치면 전부 적용. 모달 안내 문구도 "붙여넣으면 준비됐습니다 → 항목 알려주기" 2단계로 맞춘다.
...
- 추가, 편집, 개별 삭제, 일괄 삭제
- **추가 직후엔 "그 항목이 속한 도메인 목록"으로 이동한다(전 도메인 통일).** 용어→용어, 약물→약물, 미생물·공식·질환도 각자 목록으로. 과거 용어·약물은 추가 후 `finishAddFlow`를 모드 없이 불러 *직전에 보던 다른 모드 목록*에 남았다(v1.09 수정). → 추가 흐름은 `finishAddFlow(openId, mode)` **한 경로로 통일**하고 `mode`를 항상 넘긴다(`setLibraryMode(mode, true)` 후 `switchTab('search')`+`applyFilters` 1회 렌더). 도메인별로 `setLibraryMode`를 따로 또 부르면 `applyFilters`가 이중 호출되니 한 곳에서 처리.
- **카드·드로어에 "표시되는" 모든 필드는 추가·편집 폼에 "입력칸"이 있는지 확인**(표시 ↔ 편집 대칭). 실제로 약물 `brand_name`(상품명)이 카드 부제 `한국어명 · 상품명`엔 나오고 가져오기·API 보강으론 채워지는데 **추가/편집 폼엔 입력칸이 없어 보이지만 못 고치던** 케이스가 있었다(v1.07 수정). 가져오기/AI 양식/백업/해시에 포함되는 필드는 특히 폼 입력칸도 함께 둔다. (saveX가 `getElementById(id)?.value ?? 기존값`로 폼 없이도 보존하면 더 눈에 안 띄니 주의.)
