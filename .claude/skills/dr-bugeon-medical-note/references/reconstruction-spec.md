...
19. **연구노트(특허 증거 로그)는 별도 append-only 서브시스템 — 불변·격리·정정으로만 수정.** 학습 6도메인과 완전히 다른 규칙이다(상세: storage-sync §5.6/§5.6.1/§5.6.2). 재현 시 반드시: **(a) 원본 불변** — 엔트리는 `entry_hash`(SHA-256 체인, `previous_entry_hash` 포함) + ECDSA P-256 서명 + RFC3161 TSA. 저장 후 **어떤 필드도 소급 수정 금지**(고치면 서명·TSA 무효 + 변조로 보임). 수정이 필요하면 옛 것을 고치지 말고 **새 정정(correction) 엔트리**로만 표현. **(b) 사후 부착 필드는 해시 제외** — `_rnComputeHash`가 tsa 그룹·server_time·서명 그룹을 제외 → 기기별로 나중에 붙는 타임스탬프/서명이 해시를 안 깬다. 병합은 신규 id 추가 + 기존 id의 해시-제외 필드 **fill-if-empty**(§5.6). **(c) 멀티-앱 격리** — 자매 앱과 테이블을 공유하면 섞인다. `RN_PROJECT_NAME`(불변·고유, 스탬프·필터 공용)으로 `_rnLoad`·pull·매니페스트·마이그레이션 **모두에서 필터**하고 **전용 테이블**(`research_notes_med`)을 쓴다. 소유키를 `APP_INFO.name` 같은 가변값에 묶지 말 것(리네임 시 과거 기록 숨김). **(d) 끊김 경위는 주석으로** — 오염 흔적 `chain_break`는 소급 수정 대신 `rnAnnotateChainBreaks()`가 새 불변 정정 기록으로 각인(멱등=`break_set_hash`). 구조화 필드(`chain_annotation`)는 `_rnBlank`에 **조건부 스프레드**로 실어 일반 기록 해시 페이로드 불변.
