...
18. **질환 상위질환↔아형 계층(parent_id · v1.86).** 질환에 자기참조 계층 추가. **관계는 아형의 `parent_id`(nullable) 하나로만 저장**하고 부모의 아형 목록은 **역조회**(`getActiveDiseases().filter(x=>x.parent_id===id)`)로 얻는다 — 부모에 `subtype_ids` 배열 같은 양방향 저장 금지(동기화 드리프트). 새 필드라 **전 생명주기 동시 갱신**: `normalizeDiseaseForStorage`(자기참조 가드 `parentId!==selfId`)·`diseaseToRow`·`rowToDisease`·SQL(`parent_id bigint` + add column if not exists)·**스키마 버전 라벨 3곳**·자기점검(diseaseToRow가 emit하므로 드리프트/라이브DB가 자동 감지 — SQL 실행 전엔 ⚠️로 뜨는 게 정상). UI: 편집폼 "상위질환" select(`diseaseParentOptionsHtml`, 자기 제외·이름순), 드로어 상위 링크+아형 칩(`openDiseaseDrawer(event,id)`), 목록카드 "▸아형"/"아형 N" 배지(프레임당 childCount 맵 1회). **표시는 1단계 깊이만**(깊은 트리·2-cycle 방지는 미구현 — 개인 도구 수용 한계). 질환 데이터는 런타임(localStorage/Supabase)이라 index.html에 시드 없음 → 실제 콘텐츠(예: SSS)는 앱에서 입력/가져오기로만 생성.
