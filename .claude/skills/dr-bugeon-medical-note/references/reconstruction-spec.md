...
데이터는 7종(+ 흡수된 어학 도메인). 1~6번은 모두 **숫자 `id`(bigint) PK + `created_at`/`updated_at`/`deleted_at`** 공통.
1~4번(용어·약물·공식·미생물)은 **정규 컬럼형** 테이블, 5번(주요 질환)은 **하이브리드형**(핵심 플랫 컬럼 + `body jsonb`), 6번(노트)은 **JSONB형**(`data` 컬럼에 객체 통째 저장), 7번 학습상태는 별도.
> **어학(Language) 도메인(v2.52~, 폐기 어학앱 흡수):** 노트와 같은 **JSONB형**이되 **`id`는 text**(외부 앱 문자열 id 보존). 저장·정규화·동기화·백업·tombstone·방(임상 어학/CPX·OSCE)·학습(SRS)·오디오(Supabase Storage)까지 전 계층 편입 + 전역 기능(통합검색·대시보드·플래시카드) 수평전개. **상세는 §12 "어학 도메인" 항목.** 어학앱 연구노트 100건은 `_rnIsOwnProject`로 격리(레거시 증거).
...
| `dr_bugeon_language_review_log_v1` | 어학 복습 로그(로컬 · streak 집계, L3-②) |
| (Supabase) `language_records` | 어학 클라우드 동기화 테이블(jsonb, id=text, L4a) · Storage 버킷 `language-audio`(오디오, L4b) |
...
- 앱 내 데이터 관리에서 **전체 JSON 백업/복원** 제공(용어·약물·공식·미생물·주요 질환·노트·학습상태·**어학**(languageRecords/languageTombstones) 포함). 엑셀(xlsx/csv) import도 지원. 어학 백업(BOM·records)은 전용 📥복원 버튼과 표준 가져오기 양쪽 수용.
...
- `KBG_MedicalNote.*` — **플랫폼 셸(v2.34~2.48, additive namespace, 신규 코드 전용)**: 단일 HTML 내부 8-룸(Dashboard·Medical Core·Clinical Language·CPX/OSCE·Review Engine·Unified Search·Data Management·Evidence) 정보구조. 모두 **표시/라우팅 계층**이고 저장·데이터는 기존 7 테이블 그대로.
  - **진입 2형태:** 태블릿·데스크톱·**폴더블 펼침**(**≥600px**, v2.51 1101→768, v2.61 768→600 — 폴더블 펼침 670~900px가 오버레이 대신 프레임을 받도록) **상시 좌측 사이드바 + 상단바**(`Shell` — `init`/`go`/`topSearch`, `#kbgShellSidebar`/`#kbgShellTopbar`/`#kbgShellPanel`), 폰 세로(**<600px**) 헤더 **🏠 플랫폼 오버레이**(`open`/`close`/`AppRouter.go`, `#kbgPlatformOverlay`). `navTo(room)`가 현재 컨텍스트(오버레이 열림 vs 상시 사이드바) 감지해 라우팅. **셸 CSS(딱 2개 미디어쿼리 `@media(min-width:600px)`/`@media(max-width:599px)`):** `body.kbg-shell-on{padding-left:248/top:52}`로 기존 앱을 밀고 헤더는 셸에서 `position:static`. **접기(v2.64):** 상단바 ☰(`Shell.toggleCollapse`)로 `body.kbg-shell-collapsed` 토글 → 사이드바 248→64px 아이콘 레일(라벨은 `.kbg-navbtn{font-size:0}`+`.kbg-ic{font-size:18px}`로 숨김, 방 이름은 버튼 `title`로 hover 표시), padding/topbar/panel/bottom-nav left도 64로, localStorage(`kbg_shell_collapsed`) 유지·`init`에서 복원. **설정 통일(v2.64):** 헤더 툴바 `.settings-header-btn{display:none!important}` 전역 제거 — 설정은 사이드바 하단 ⚙️/상단바 ⚙️(셸) 또는 🏠 오버레이 설정(모바일)로만(세 경로 동일 `showAISettings`). **⚠️ 화면 밖 fixed 드로어 가로스크롤 함정:** 셸 padding 때문에 화면 밖(닫힘) 상세 드로어가 `html`(overflow-x:visible)에서 문서를 가로로 늘려 좁은 셸 폭에서만 가로스크롤 발생 → **min-width 셸 블록 안**의 `html{overflow-x:hidden}`로 클립(분기점 바꾸면 이 값도 같이). 검증 매트릭스에 분기점 경계(599/600)를 반드시 포함.
...
- **어학 도메인(Language) — L2 저장 계층(v2.52, additive IIFE `installLanguageDomain`)**: 폐기 예정 어학앱의 복원 데이터를 흡수하는 별도 로컬 도메인(JSONB-blob, 노트와 동일 구조). `LANGRECORDS`(window 노출)·`normalizeLangRecord`(원본 16필드 무손실 흡수: `audio_url↔audioUrl`·`updated_at↔updatedAt` 리네이밍 외 항등, `learning{level,needCheck,starred,updatedAt}` verbatim, id 문자열 그대로)·`langToBackupRecord`(역변환)·`loadLanguageRecords/saveLanguageRecords/bootstrapLanguageRecords`(large-cache/IDB)·`importLanguageBackupData(input,{mode})`(**BOM 안전** — 어학 full backup `records` + 자체 `languageRecords` 양쪽 수용, mode replace/merge/append)·`exportLanguageBackupData`(어학형식 왕복)·`window.__langRoundTripSelfTest`(무손실 self-check). 전체 백업 편입: `createCompleteBackupObject`를 **다시 감싸** `backup.languageRecords/languageTombstones/languageMeta` 추가(노트 패치 위 체이닝). tombstone: `getLanguageTombstoneCount/clearLanguageTombstones/pruneLanguageTombstones`(window) + 3함수(`getProtectedRecordCounts`·`confirmClearAllProtectedRecords`·`pruneAllProtectedRecords`)에 방어적 등록(불변조건 13). **L3-①(v2.53):** 방 렌더 — `AppUI.roomClinicalLanguage`/`roomCpxOsce`가 `window.LANGRECORDS`를 방별 분류(`_langIsCpx`: type CPX·OSCE 또는 subtype CPX_CASE → CPX/OSCE, 그 외 → 임상 어학)로 표시. `_langScoped`/`langListHtml`(검색·CAP 300)·`langRenderList`(라이브검색)·`langOpenDetail`(읽기전용 상세 모달)·`langRestorePrompt`(전용 📥복원 버튼, `importLanguageBackupData` 경유)·`langRefreshRooms`(복원 후 현재 방 재렌더). 데이터 없으면 복원 CTA+미리보기(빈 상태). **표준 복원 UI 배선(L3-①b, v2.54 — "둘 다 되게"):** 방 📥버튼(`langRestorePrompt`)과 **표준 "데이터 관리 → 가져오기"** 양쪽으로 복원. 언어 IIFE가 4개 함수 재래핑: `readDataImportFile`(어학 백업 **구제 rescue** — 원본이 throw하면 catch 후 BOM-aware 재파싱, `languageRecords`(자체 백업) 또는 `records`+`format=dr-bugeon-language-full-backup`/`_looksLikeLangRecords`(어학앱 전용) 감지 → 빈 의학 도메인+`languageRecords`+`languageOnly:true` 페이로드로 구제; 원본 성공 시엔 `languageRecords`만 passthrough)·`calculateImportPreview`+`importPreviewCountsHtml`(미리보기에 "어학 추가 N")·`applyPendingDataImport`(원본 호출 **전** `pendingDataImport.languageRecords`·`languageOnly` 캡처 — `languageOnly`면 용어 중심 원본 파이프 건너뛰고 어학만 복원해 "용어 0개" 오해 방지, 아니면 원본 후 편입; 노트 패치와 동일 캡처 패턴). **데이터는 사용자가 앱 복원기능으로 직접 주입 — 앱은 빈 상태로 배포(방 열면 복원 CTA).** **L3-②(v2.55):** 학습기능(어학앱 로직 이식, 클라이언트 사이드·`learning` 상태 저장·무손실). SRS: `LANG_REVIEW_STEPS=[0,1,3,7,14,30]`·`langReviewDueDate`/`langIsDue`/`langReviewQueue`/`langDueCount`·`langMarkReview(id,'good'|'hard')`(레벨±1 캡0~5+`reviewedAt`+복습로그)·`langToggleStar`/`langToggleNeedCheck`·`langSetLearning`(공통 저장)·복습로그(`dr_bugeon_language_review_log_v1`, 로컬 날짜키)·`langReviewStreak`. UI: `_langLearnBar`(별표/확인필요/다시·외움)·`_langMediaBar`(🔊영어 듣기=기존 `speakText` 재사용, 녹음 재생/다운로드/삭제)·`langOpenDetail`+`_langDetailInner`/`_langDetailRerender`·`langAction`(액션→저장→상세·목록·복습 갱신)·`langReviewOpen`/`_langReviewStep`/`langReviewGrade`(답 보기→채점 SRS 큐, 방 scope별). 녹음 캡처: `langRecordStart`/`langRecordStop`(MediaRecorder, `_langBestMime` webm/opus 우선, 미리듣기 blob). `langPlayAudio`/`langDownloadAudio`/`langDeleteAudio`(`langSetAudioUrl`). **경계:** 녹음 **캡처·재생까지** L3-②; **업로드(영속화)는 L4 — Medical Note Supabase Storage**(캡처 blob은 세션 한정, 기존 audioUrl은 폐기 어학앱 저장소라 재생 실패 가능). **L4a(v2.56):** Supabase 동기화 — 노트(jsonb-blob) 패턴 그대로 미러링, **단 `language_records.id`는 text**(문자열 id 보존). `langToRow`(`{id:String, data:jsonb, updated_at, deleted_at}`)/`rowToLang`·`dedupeLangById`·`sbLoadAllLanguage`/`sbBatchUpsertLangRows`/`sbDeleteLang`·`mergeLanguageForSync`(제네릭 `pickSyncWinner`/`termSyncTime` 재사용, 링크 없어 union 불필요·최신 updatedAt 우선)·`pushLanguageToCloud`/`pullLanguageReplaceFromCloud`/`syncLanguageWithCloud`. 전역 3진입점(`sbPushAll`/`pullFromCloud`/`syncWithCloud`)을 다시 감싸 체이닝(노트 wrap 위). 🔴 **안전: `_langLoaded` 가드**(미로드 시 push 금지=빈 배열 대량삭제 방지)·**테이블 없으면 sbLoad→null→inert**(graceful, 크래시·삭제 없음)·삭제는 tombstone 병합 보존. **통합 SQL 한 방**: in-app `SUPABASE_SCHEMA_SQL`에 `language_records`(id text pk·data jsonb·인덱스) + **`language-audio` Storage 버킷(공개 읽기)+정책** + 진단 SELECT 추가. jsonb라 check-index parity(REQUIRED_TABLES)·schema/restore-drift 대상 아님(notes와 동일 제외). 단위검증 `langToRow/rowToLang` 왕복 2013/0·미설정 graceful(라이브는 사용자가 SQL 실행 후 확인). **L4b(v2.57):** 오디오 Supabase Storage(`language-audio`) 연동 — 어학앱 `uploadAudioToStorage` 패턴 이식(JSZip 없음 → 다중 파일 선택으로 ZIP 대체). `sbUploadLangAudio`(POST `storage/v1/object/{bucket}/{safeName}`, MIME 정규화·ASCII 파일명·x-upsert → public URL)·`sbDeleteLangAudio`·`langAudioFileName`(`ID-{recordId}_{slug}.{ext}` — 왕복 매칭)·`langRecordIdFromFileName`(`/ID-(.+?)_/` — 자체+ZIP `NNN_ID-{recordId}_…` 모두)·`langIsOwnAudioUrl`. UI: 캡처 미리듣기에 "☁ 저장(업로드)"(`langUploadCapturedAudio`→`langSetAudioUrl`)·방 상단 "🎙 녹음 이관"(`langAudioMigratePrompt` 다중선택→recordId 매칭→업로드→audioUrl)·삭제 시 우리 버킷 원본 정리(타 레코드 참조 시 연결만 해제). 단위검증 ZIP 55/55 파싱·매칭·미설정 graceful(라이브 업로드는 사용자가 SQL+설정 후). **어학 통합 L1~L4 완료.**
  - **전수감사 후속(v2.58, 멀티에이전트 6차원+적대적 재검증 확정 8건 수정):** ① replace 임포트가 제거분에 tombstone 미생성 → `importLanguageBackupData` replace 분기에 `removed…map({deletedAt})→saveLangTombstones` 추가(부활 방지, 의학 미러). ② 백업 `languageTombstones` export만 하고 복원 미읽기 → `parseLangTombstones` + 3임포트 경로(readDataImportFile passthrough·applyPendingDataImport 캡처·importLanguageBackupData 병합)에서 되읽어 active에서 제거. ③ `normalizeLangRecord`/`langToBackupRecord`가 `learning.reviewedAt` 누락 → **조건부** 보존(없던 레코드엔 키 미추가로 왕복 byte-identity 유지). ④ TTS onclick 큰따옴표 속성에 `JSON.stringify(r.english)` 보간 → 전 항목 파손 → `AppUI.langSpeakEnglish(ev,id,type)`(id만 넘겨 핸들러 조회)로 교체. ⑤ 문자열 id를 작은따옴표 onclick에 넣을 때 `'` 미이스케이프 → `KBG_MedicalNote._j(v)`(=`JSON.stringify` 후 `'`→`&#39;`)로 전 어학 id 삽입 교체. ⑥ `langUploadCapturedAudio` 재업로드가 공유 URL stillUsed 미검사 삭제 → `langDeleteAudio`와 동일 가드 추가. ⑦ `showSavedVersionInfo`에 어학 누락 → `window.appendLanguageVersionInfo`(노트 훅 미러, 진단) 추가. (커버리지 gap grep 확인: tombstone 3함수·전역 sync 3진입점·SQL은 이미 편입됨.)
  - **자동 안전망 확장(v2.58 후속):** 어학도 다른 도메인처럼 게이트가 지키도록 확장 — `golden-tests.mjs`에 **`normalizeLangRecord` 골든 잠금**(무손실 왕복·audioUrl null 보존·id 문자열·level 숫자화·**reviewedAt 조건부** 회귀 고정; 기본파라미터 `(r={})` 때문에 파라미터 리스트 건너뛰는 전용 추출기 사용) 추가, `check-index-scripts.mjs` **REQUIRED_SYMBOLS**에 어학 코어 9종(`installLanguageDomain`·`normalizeLangRecord`·`importLanguageBackupData`·`langToRow`·`syncLanguageWithCloud`·`sbUploadLangAudio`·`roomClinicalLanguage`·`langOpenDetail`·`langSpeakEnglish`) 추가(삭제/리네임 시 CI 빨간불). **⚠️ 저장소 역할: Supabase=텍스트+오디오 녹음(Storage 버킷), Cloudinary=이미지+영상**(오디오는 Cloudinary 아님 — 원본 어학앱도 오디오를 자기 Supabase Storage에 뒀고, 텍스트와 같은 백엔드에 둬야 인증·수명주기·백업 통일)(지금 신규 테이블·`*ToRow/rowTo*` 없음 → check-index parity·schema/restore-drift 대상 아님). **🔴 어학앱 연구노트 100건은 이 경로로 넣지 않음** — `project_name='Dr 김부건의 언어 마스터를 위한 여정'`이라 `_rnIsOwnProject`가 자동 격리, 레거시 증거로 보존(법적효력 유지). 상세: `docs/KBG_MedicalNote_Language이식_L1_임포터_왕복검증.md`.
