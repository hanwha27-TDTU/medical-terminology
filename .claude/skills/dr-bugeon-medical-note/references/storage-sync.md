...
연구노트(전용 테이블 `research_notes_med` — v2.03 격리, §5.6.1)는 일반 도메인과 **동기화 규칙이 다르다**. 원본 불변·append-only라 "덮어쓰기/교체" 개념이 없고, id 기준 **union 병합**(기존 삭제 없음)만 한다. 상세 구현은 ui-features.md 연구노트 클라우드 백업 항목 참조. 동기화 관점 핵심만:
