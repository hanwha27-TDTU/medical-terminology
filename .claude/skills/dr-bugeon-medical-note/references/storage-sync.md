...
### 15.8 노트 링크 편집 보존 (⚠️AUDIT 신규)

1. 기기 A에서 노트의 `linkedDrugs`(또는 microbes/formulas)만 수정.
2. 기기 B가 같은 노트를 더 최신 `updatedAt`으로 들고 있는 상황에서 동기화.
3. A의 링크 편집이 "(충돌 사본)"으로라도 보존되는지 확인한다(`noteContentDiffers`가 link 4종 비교 — §6.1).
