...
- **클라우드 백업(v1.69):** `research_notes` 테이블은 **jsonb-blob 방식** — 엔트리 전체를 `data(jsonb)`에 통째 저장(+ id/created_at/app_version/event_type/entry_hash/previous_entry_hash/updated_at 고정 컬럼). 엔트리 필드가 늘어도 **컬럼 스키마가 안 바뀌어 드리프트 없음**. append-only라 push=upsert(`researchNoteRow`), pull=**id 기준 union 병합**(기존 삭제 없음, created_at 정렬). 새 기록 저장 시 `rnPushOne` fire-and-forget 자동 백업. **사후 부착 필드 기기간 전파(v1.82):** union 병합이 "로컬에 없는 것만 추가"라, 한 기기에서 받은 TSA 토큰·서명·server_time이 그 기록을 이미 가진 다른 기기로 안 내려오는 버그가 있었다 → `rnPull`이 **기존 로컬 기록에도 해시-제외 사후필드(tsa 그룹·server_time·서명 그룹)를 로컬이 비었을 때만** 클라우드에서 fill(원문·해시 불변이라 검증 영향 0). `enriched` 카운트 토스트. **원칙: 원문/해시는 병합 대상 아님, `_rnComputeHash` 제외 필드만 fill-if-empty**(그래야 "윈도우에서 1회 받고 다른 기기는 합치기만" 성립). **자동 동기화(v1.83):** `rnOpenNotebook`이 genesis 후 `_rnAutoSync()`(fire-and-forget) 호출 → 열 때마다 자동 `rnPull`로 토큰 전파(사용자 클릭 불필요). 20초 throttle(`_rnLastAutoSync`, 앱이라 `Date.now()` OK), added/enriched 있을 때만 toast. fill-if-empty라 **기존에 기기별로 따로 받은 토큰은 안 덮음**(같은 해시의 복수 유효 도장=무해). 새 기록은 `_rnAttachTsa`(저장 시) 1회 + 자동 pull로 전 기기 통일. 전체 JSON 백업(`createCompleteBackupObject.researchNotes`)에도 포함하고 `applyPendingDataImport`가 모드 무관 union 병합. **다기기 병합 시 체인 링크(prev)는 불연속일 수 있음** → 검증은 `hash_mismatch`(변조=심각)와 `chain_break`(병합 시 정상 가능=정보)를 **구분해 안내**(per-entry 해시가 강한 보증). 새 테이블이라 **스키마 버전 라벨 3곳 bump + 자기점검(computeSchemaDrift·checkLiveSchema) 도메인에 research_notes 추가**(§8.1.1). 저장은 여전히 localStorage 우선(오프라인) + 클라우드는 백업. **⚠️현재 상태(v2.03~2.04): 이 테이블은 자매 앱과의 섞임 때문에 전용 테이블 `research_notes_med`로 이사했고(프로젝트 필터 격리), `rnOpenNotebook`은 genesis 대신 `_rnBootstrapNotebook`(즉시 렌더→1회 마이그레이션→genesis→autoSync)을 호출하며, 오염 시기 체인 끊김은 `rnAnnotateChainBreaks`로 경위를 주석한다. 위 메커니즘(jsonb-blob·fill-if-empty·자동동기화)은 그대로 — 표 이름·격리·부트스트랩만 바뀜. 상세: storage-sync §5.6.1/§5.6.2.**
