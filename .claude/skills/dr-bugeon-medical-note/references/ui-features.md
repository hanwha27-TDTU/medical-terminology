...
- **통합 지시문 앱 내장(v1.80):** 연구노트 시스템 전체 이식 지시문을 앱에 **`<script id="rnPatentInstructionDoc" type="text/markdown">` 비실행 데이터 블록**으로 내장, `rnDownloadInstructionDoc`가 `textContent` 읽어 .md 다운로드("📄 연구노트 지시문 받기" 버튼). **핵심 주의:** 지시문에 **`</script>`가 0개**여야 안전(HTML은 `</script`에서만 블록 종료 — 백틱·`${}`·닫힘없는 `<script`는 무해). 삽입 전 `grep -c "</script" 지시문` 필수. **검증 추출 로직 보강:** `<script>` 블록 node --check 시 `type=`가 js/module/빈값이 **아니면 skip**(안 하면 markdown을 JS로 검사해 FAIL). 삽입은 **Node 주입 스크립트로 파일→블록 교체**(수동 복붙 금지, 대용량·특수문자 안전). 내장본이 canonical — 지시문 갱신 시 이 블록도 함께 갱신. 앱 실행 영향 0.
