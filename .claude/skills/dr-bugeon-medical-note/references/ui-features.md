...
## 🎴 플래시카드(전 도메인 능동 회상, v2.31~) — "원본 불변 · view만 생성"

- **원칙: 플래시카드는 원본(terms/drugs/formulas/microbes/diseases/notes)을 절대 안 바꾼다.** 원본을 **읽어서** 카드 view(앞/뒤)만 만들고, 평가 기록은 **별도 로컬** `flashcardStats`(키 `FLASHCARD_STORAGE_KEY`)에만 저장 → 동기화·백업·tombstone 구조 무영향(불변조건 2·17과 일치). 새 도메인/필드가 늘어도 플래시카드는 sync/backup parity 대상이 아니다.
- **cardId = `domain:id`(예 `drug:7`)로 안정 고정** — 기록 섞임 방지. 도메인별 `buildFlashcardPool`의 card builder가 앞면(대표 이름)·뒷면(도메인별 필드: 약물=MOA·부작용, 미생물=그람·치료·usmle_clues, 질환=요약·본문, 노트=틀린 이유·정답 등)을 만든다. **빈 뒷면 카드는 pool에서 제외**, 빈 필드행은 자동 생략.
- **기존 SRS/퀴즈를 재발명하지 말 것** — term 학습상태(dueAt·studyStatus, 클라우드 동기화)와 퀴즈(due/weak scope)는 이미 있다. 플래시카드는 **6도메인 확장 + 앞/뒤 UX**만 얹은 것(Tier 1: 랜덤+평가·로컬). 가중·SRS-lite는 `flashcardStats` 구조를 확장하되, **term 학습상태 스키마(동기화됨)는 건드리지 않는다**.
- 실행: 자체 모달(`#flashcardModal`)로 자체 렌더 루프(`renderFlashcard`) — 퀴즈의 mainContent/utility-modal 스냅샷 메커니즘에 얽히지 않게 분리. 진입은 📝 학습 퀴즈 화면의 "🎴 플래시카드" 런처. 필수 심볼: `buildFlashcardPool`·`showFlashcards`·`renderFlashcard`(check-index-scripts).
...
- **카드 버튼 직관화(v1.84):** 정정 계열 버튼이 "상태 태그"처럼 보여 "누르면 뭐가 되나" 혼동 → **행동형 라벨**(🔎 상세보기 / 🏷️ 특허감으로 표시 / ＋ 내 결정 덧붙이기 / ↻ 정정(수정본 추가) / 🗑️ 삭제표시) + **버튼별 `title` 툴팁**(동작 한 줄) + 버튼 위 **안내 한 줄**("🔒 원본 못 고침 — 원본 보존한 채 연결된 새 기록 추가"). `rnOpenForm`의 corrNote를 `correction_type`별로 구체화. **교훈: append-only/정정 UI는 "무엇이 되는가"가 라벨만으로 안 보이면 파괴적 동작으로 오해할 수 있다 → 라벨은 행동형, 공유 원칙은 근처 한 줄, 세부는 툴팁.**
...
- **CSV는 UTF-8 BOM 필수(v1.85):** 연구노트 CSV를 윈도우 Excel에서 열면 한글이 CP949로 오독돼 깨짐 → `rnDownload`가 mime/확장자가 csv면 **앞에 `﻿`(BOM) 부착**(이미 있으면 skip). 파일은 원래 UTF-8 정상이고 Excel이 자동인식을 못 하던 것(데이터·JSON·MD는 멀쩡). **앱의 다른 CSV(`downloadBlobFile('﻿'+csv…)`)는 이미 BOM을 붙이는데 연구노트 `rnDownload`만 누락됐던 케이스** — 새 CSV 다운로드 경로를 만들면 BOM 규칙을 잊지 말 것(JSON/MD엔 붙이지 않음).
- **클라우드 백업(v1.69~) — 연구노트 전용 테이블 `research_notes_med`(jsonb-blob).** 엔트리 전체를 `data(jsonb)`에 통째 저장(+ id/created_at/app_version/event_type/entry_hash/previous_entry_hash/updated_at 고정 컬럼) → 엔트리 필드가 늘어도 **컬럼 스키마 불변(드리프트 없음)**. append-only라 push=upsert(`researchNoteRow`), pull=**id 기준 union 병합**(기존 삭제 없음, created_at 정렬). 새 기록 저장 시 `rnPushOne` fire-and-forget 자동 백업. **사후 부착 필드 기기간 전파(v1.82):** union 병합이 "로컬에 없는 것만 추가"라 한 기기에서 받은 TSA·서명·server_time이 이미 그 기록을 가진 다른 기기로 안 내려오던 버그 → `rnPull`이 **기존 로컬 기록에도 해시-제외 사후필드(tsa·server_time·서명 그룹)를 로컬이 비었을 때만** fill(원문·해시 불변, 검증 영향 0). **원칙: 원문/해시는 병합 대상 아님, `_rnComputeHash` 제외 필드만 fill-if-empty**(기존 기기별 토큰은 안 덮음=같은 해시의 복수 유효 도장). **자동 동기화(v1.83):** 노트북 열 때 `_rnAutoSync()`(fire-and-forget, 20초 throttle `_rnLastAutoSync`) 자동 `rnPull` → 사용자 클릭 없이 토큰 전파, added/enriched 있을 때만 toast. 전체 JSON 백업(`createCompleteBackupObject.researchNotes`) 포함 + `applyPendingDataImport` union 병합. 저장은 localStorage 우선(오프라인)+클라우드 백업. **격리·부트스트랩·검증(v2.03~2.05, 상세: storage-sync §5.6.1/§5.6.2 + 아래 v2.05 불릿):** 자매 앱과 `research_notes` 공유로 섞이던 것을 전용 테이블 `research_notes_med`+프로젝트 필터로 격리, 노트북 열기는 `_rnBootstrapNotebook`(즉시 렌더→1회 마이그레이션→genesis→autoSync), 오염 시기 체인 끊김은 `rnAnnotateChainBreaks` 경위 주석, 해시 검증은 결과 모달. 자기점검(computeSchemaDrift·checkLiveSchema) 도메인 테이블도 `research_notes_med`.
- **⚠️AUDIT 결과는 토스트가 아니라 "결과 화면"으로, 확인창은 트리거 모달 위 레이어로(v2.05).** `rnRunVerify`가 검증 결과를 **토스트로만** 잠깐 띄우고 사라져 "결과 화면이 안 뜬다"는 리포트 + 수백 건 검증(각 `_rnComputeHash`+`_rnVerifySig`)에 몇 초 걸리는데 **로딩 피드백이 없어** 멈춘 것처럼 보였다. 픽스: `rnOpenVerifyModal`(로딩 상태 먼저) → `rnVerifyChain` await → `rnVerifyResultHtml`로 상세(통계 4칸: 내용해시·전자서명·TSA·체인링크 + 문제 기록 목록: 어느 기록/시각/EC번호/끊긴 previous). 체인 불연속이면 결과 화면에서 바로 `rnAnnotateChainBreaks()`로 이어가는 버튼. **일반 교훈: 시간이 걸리는(await) 액션은 결과를 영속 UI(모달/패널)로 보여주고, 시작 즉시 로딩 상태를 렌더해 "무반응"을 없앤다. 요약 토스트는 보조.** **z-index 함정: `showConfirm`(당시 999)이 `.developer-modal-overlay`(1200)·연구노트/트리 모달보다 아래라, 그 모달들 안에서 띄운 확인창이 뒤에 숨어 클릭 불가였다(예: "끊김 경위 주석" 확인창이 안 보임) → showConfirm을 1900으로 올림. 원칙: 다른 오버레이 위에서 트리거되는 확인/알림창은 항상 그 오버레이보다 높은 z-index여야 한다(새 전역 오버레이 추가 시 showConfirm 층과 비교).** 검증 자체 로직(`hash_mismatch`=변조·심각 / `signature_invalid` / `chain_break`=오염흔적·정보)의 구분 안내는 유지.
...
- **TSA 상태·Evidence Chain ID·openssl 명령(v1.77 · 전문가 2차 검토):** ⑤② **`tsa_status`(pending/success/failed)+`tsa_error`** — `_rnComputeHash` 제외(가변). 저장 시 `getSupabaseConfig()?'pending':'unavailable'`, `_rnAttachTsa`가 성공/실패로 갱신(`_rnRequestTsa`는 이제 `{ok,error}` 반환). 카드 배지 색 구분(⏱️공인시각/TSA대기/TSA실패), 실패 사유 title·MD에 노출 → "실패=조용히 무시" 폐기. ⑥ **Evidence Chain ID `EC-YYYYMMDD-NNNNNN`**(`_rnMakeEcId` — seq=기존 ec 보유 수+1) — 생성 시 고정이라 **해시 포함**(불변). 카드🔗·MD 헤더에. UUID가 진짜 키, EC는 사람용 라벨(다기기 병합 시 라벨 중복 가능하나 무해). ④ **openssl 검증 명령 자동생성 `_rnOpensslCmd`** — 토큰 base64 heredoc→`base64 -d`→`openssl ts -verify -digest <hash> -in note.tsr -CAfile tsa_ca.pem`. 상세에 "📋 복사" 버튼(`rnCopyOpensslCmd`, clipboard→실패 시 .sh 다운로드), MD에도 포함. ① freeTSA 기본은 유지(제로설정 테스트) — **정정:** TSA 토큰은 기관이 문 닫아도 CA 인증서만 아카이브하면 계속 검증됨(서비스중단=신규발급만 불가). 장기보존은 상업 TSA+CA 아카이브 권장(가이드). ③ PDF/git 트리플 타임스탬프는 보류(git은 GitHub가 이미 타임스탬프, PDF는 인쇄생성이라 안정 바이트 없음). 새 필드 전부 `data jsonb`라 스키마 불변.
- **공개용 해시 매니페스트(v1.78 · public disclosure 방지):** 공개 레포(GitHub Pages)에 연구노트 **원문**을 올리면 출원 전 "공개 공지 → 신규성 상실" 위험. **정정:** 이 앱은 원래도 원문을 레포에 안 올린다(원문=localStorage+본인 Supabase, 앱엔 git/GitHub API/Release 코드 없음, `.github/workflows` 없었음). 이 기능은 "공개로 존재 증명하고 싶을 때"의 안전 수단. **핵심 원칙: 화이트리스트만**(`RN_MANIFEST_FIELDS` = id·evidence_chain_id·created_at·server_time·app_version·event_type·entry_hash·previous_entry_hash·서명 3필드·TSA 5필드). **블랙리스트 금지**(새 필드 생기면 원문이 샘). `_rnManifestEntry`가 허용 키만 pick → feature_name·problem_statement·human_idea·ai_*·patent_*·files·witness·발명자 개인정보 전부 제외. `rnExport('manifest')` 버튼 "🔓 해시 매니페스트(공개용)". **서버측(GitHub Actions `research-manifest.yml` + `scripts/build-research-manifest.mjs`):** Supabase read→동일 ALLOW로 strip→**허용목록 밖 키 발견 시 process.exit(1)** 이중 안전장치→공개 Release 첨부(수동 dispatch, 시크릿 SUPABASE_URL/KEY). 두 allowlist는 반드시 동일 유지. **검증: Node로 원문 필드 제외 실측**(feature/problem/human_idea/ai_full/patent/inventor/files 유출 0 확인). TSA 토큰·공개키는 비밀 아니라 매니페스트 포함 OK. inventor_name 등 PII는 보수적으로 제외.
- **통합 지시문 앱 내장(v1.80):** 연구노트 시스템 전체 이식 지시문을 앱에 **`<script id="rnPatentInstructionDoc" type="text/markdown">` 비실행 데이터 블록**으로 내장, `rnDownloadInstructionDoc`가 `textContent` 읽어 .md 다운로드("📄 연구노트 지시문 받기" 버튼). **핵심 주의:** 지시문에 **`</script>`가 0개**여야 안전(HTML은 `</script`에서만 블록 종료 — 백틱·`${}`·닫힘없는 `<script`는 무해). 삽입 전 `grep -c "</script" 지시문` 필수. **검증 추출 로직 보강:** `<script>` 블록 node --check 시 `type=`가 js/module/빈값이 **아니면 skip**(안 하면 markdown을 JS로 검사해 FAIL). 삽입은 **Node 주입 스크립트로 파일→블록 교체**(수동 복붙 금지, 대용량·특수문자 안전). 앱 실행 영향 0. **자동 동기화(v1.81):** 단일 원본은 **`docs/특허연구노트_통합지시문.md`** 파일(내장 블록이 아니라 이 파일이 canonical) → `scripts/sync-instruction-doc.mjs`가 원본을 내장 블록에 주입(`--check`는 어긋나면 exit 1). **GitHub Actions `instruction-doc-sync`**가 index.html·원본·스크립트 변경 시 `--check` 실행해 drift를 자동 차단. **규율: 지시문은 원본 파일만 고치고 `node scripts/sync-instruction-doc.mjs` 실행 후 커밋**(내장 블록 손수정 금지 — CI가 잡는다). 정규화: 내장 inner = `\n`+원본(앞뒤 빈줄 trim)+`\n`.
- **지난 이력 소급 기록(v1.69→증분 v1.79):** `UPDATE_HISTORY`(앱에 v1.01~ 전 이력 존재)를 `rnImportUpdateHistory`로 연구노트에 소급 변환. **백데이팅 금지** — created_at=지금, `source:'update_history'`+`retrospective:true`+`describes_version`으로 "버전 로그 재구성"임을 명시(특허 증거에서 날짜 조작은 신뢰를 깎음). 실제 타임라인 권위 원본은 git 커밋(실제 날짜+SHA)이라고 노트에 남긴다. **v1.79 증분화:** 전엔 "이미 `source==='update_history'` 있으면 전체 skip"이라 첫 실행 후 새 버전이 영영 안 들어왔다 → `describes_version`(없으면 title) 키 집합 `have`로 **이미 가져온 버전만 건너뛰고 새 버전만 추가**. 여러 번 눌러도 최신까지 계속 반영. (연구노트 자동로깅 한계: 브라우저 앱이라 외부 AI 대화·코드편집·git을 못 봄 → "AI로 개발 = 버전 bump + UPDATE_HISTORY 한 줄" 규율 + 이 증분 import가 유일한 현실적 다리. git이 진짜 자동 로그.)
...
- **넓은 하위선택자(`.x span`)에 새 속성 추가 시 주의(v1.98):** `.microbe-memory-line span`은 라벨 알약 스타일이지만 **모든 하위 span**(📖 아이콘·하위칩 컨테이너 등)에 걸린다. 라벨 정렬용 `min-width`를 여기 넣으면 아이콘·칩까지 46px가 됨 → 라벨만 겨냥하려면 **`.microbe-memory-line > span:first-child`**(직계 첫 자식)로 좁힌다. 카드 라벨(한 줄/주호소/정의 등)이 글자수로 폭이 달라 본문 시작점이 계단처럼 어긋나던 것 → 라벨에 `min-width:46px`+`justify-content:center`+`box-sizing:border-box`로 동일폭 정렬. **넓은 element 하위선택자에 속성 추가 전, 그 안에 다른 span/자식이 있는지 확인하고 필요하면 `:first-child`/전용 클래스로 스코프.**
...
- **모드 무관 "통합 추가" 버튼은 유형 선택 메뉴로 6개 도메인을 한 진입점에서 처리한다(v2.00).** 툴바(`.top-tool-row`, NIH UMLS 검색 옆)의 `➕ 추가` 버튼은 지금 보고 있는 라이브러리 모드와 무관하게 6종(용어·미생물·약물·공식·질환·개념 오답 노트)을 추가한다. `openUniversalAddMenu()`가 중앙 모달(`openAIContentTemplateMenu` 패턴 재사용 — `overlay-bg`+`e.target===modal`로 바깥클릭 닫기)로 유형 목록을 띄우고, `universalAdd(kind)`가 라우팅한다: 노트는 별도 JSONB 편집창(`openIntegratedNoteModal()`)이라 그쪽으로, 나머지 5종은 `showAddTermModal({forceLibraryAdd:true})`로 라이브러리 추가창을 **강제로 연 뒤**(패치 래퍼가 `currentLibraryMode==='notes'`일 때 노트창으로 새는 것을 `forceLibraryAdd`로 우회) `switchAddKind(kind)`로 원하는 유형으로 전환한다. `showAddTermModal`은 항상 현재 모드 기준으로 kind를 먼저 세팅하므로 그 뒤 `switchAddKind`로 덮어써야 임의 유형 선택이 된다(동기 실행이라 순서 보장). 기존의 모드별 목록 상단 추가 버튼(`.inline-add-btn`)은 그대로 유지 — 두 진입점 공존. 버튼은 공용 `.ui-btn.sm`에 `.add-any-btn`(accent 배경)만 얹어 눈에 띄게.
...
- **⚠️AUDIT(대칭): 색인(용어 등록명)과 질의(소견 토큰)는 반드시 같은 분해 규칙을 써야 한다(v2.01).** v1.59에선 질의 측 `findTermIdByFinding`만 괄호를 앞/안으로 갈랐고, 색인 측 `buildFindingConceptIndex`는 등록명을 통째로만(쉼표·세미콜론·슬래시·가운뎃점 분리 후) `normFindingConcept`로 넣었다. 그래서 용어가 **"두근거림 (심계항진)"처럼 괄호 안에 동의어를 담아 한 문자열로** 등록되면 색인 키가 `두근거림심계항진` 하나뿐이라, 바른 소견 토큰 "두근거림"(정규화 `두근거림`)이 색인에 없어 📖가 안 붙었다(사용자 리포트: 용어는 존재하는데 주호소가 인식 못 함). 쉼표 분리 자체는 정상이었고 원인은 이 **비대칭**. 픽스: 공용 `_findingConceptForms(s)`(통째·괄호 앞·괄호 안)로 **양쪽 모두** 분해 → 색인이 `두근거림심계항진`·`두근거림`·`심계항진` 세 키를 모두 등록, 질의도 같은 헬퍼 사용. 교훈: "한쪽만 정규화/분해" 패턴은 이런 매칭 코드에서 반복 결함 — 색인·질의는 항상 같은 헬퍼를 공유하라. (Node 재현 테스트로 current=null→fixed=id 확인 후 반영.)
- **⚠️AUDIT(가운뎃점 대칭): 색인의 ko 분리 규칙과 소견 토큰 분리 규칙이 달라 "오심·구토" 유형이 안 걸렸다(v2.02).** `buildFindingConceptIndex`는 용어 ko를 `/[,;/·]/`(쉼표·세미콜론·슬래시·**가운뎃점**)로 갈라 색인했는데, 소견 토큰 쪽 `_findingSplit`은 의도적으로 **쉼표·줄바꿈으로만** 갈라 "오심·구토(Nausea/vomiting)" 같은 한 용어를 통째로 보낸다(v1.58 계약). 그래서 용어가 "오심·구토 (…)"로 등록되면 색인엔 `오심`·`구토…` 조각만 있고 **통째 `오심·구토`가 없어** 소견 "오심·구토"가 매칭 실패. 픽스: 색인에 **통째 ko를 먼저 push**한 뒤 `·`/쉼표 분리 조각도 push(둘 다 색인) → "오심·구토"는 통째로 걸리고, "실신·기절"처럼 ·를 동의어 구분자로 쓴 경우도 각각·통째 모두 걸린다(키만 추가라 회귀 없음, `!idx.has(n)` 선등록 우선 유지). 일반 원칙: **여러 분리 규칙이 공존하면(용어 ko=·분리, 소견=쉼표분리) 색인은 "가장 안 쪼갠 형태"까지 포함해 양쪽 규칙의 합집합을 담아라.**
