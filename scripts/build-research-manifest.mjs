...
// Supabase의 research_notes를 읽어 "해시 전용 매니페스트"를 만든다. 발명 원문은 0% — 화이트리스트 필드만.
...
const res = await fetch(`${url.replace(/\/$/, '')}/rest/v1/research_notes?select=data&order=created_at.asc&limit=100000`, {
  headers: { apikey: key, Authorization: `Bearer ${key}` },
});
if (!res.ok) { console.error('❌ Supabase read 실패', res.status, await res.text()); process.exit(1); }
const rows = await res.json();
const notes = (Array.isArray(rows) ? rows : []).map(r => r.data).filter(x => x && x.id);
